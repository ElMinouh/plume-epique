...
function navigateToLink(targetType,targetIdx){const tabId=targetType==='chars'?'tab-chars':targetType==='places'?'tab-places':'tab-quests';const btn=document.querySelector(`button[data-tab-id="${tabId}"]`);if(btn)toggleTab(tabId,btn);if(targetType==='quests')showQuestEdit(targetIdx);else showEdit(targetType,targetIdx);}
