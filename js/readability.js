...
function renderAnalytics() {
  ...
  document.getElementById('analytics-bars').innerHTML = db.chapters.map((ch,i) => {
    ...
    return `<div class="chapter-bar-row"><span class="u-minw-70px u-ov-hidden u-tovf-ellipsis u-ws-nowrap" title="${DOMPurify.sanitize(ch.title)}">Ch.${i+1}</span>
      <div class="u-fg-1 u-bg-v-item-bg u-br-4px u-h-8px u-ov-hidden">
        <div class="chapter-bar-fill" data-pct="${pct}"></div></div>
      <span class="u-minw-45px u-ta-right u-fs-_7rem">${wc} m.</span></div>`;
  }).join('');
  document.querySelectorAll('#analytics-bars .chapter-bar-fill[data-pct]').forEach(el => {
    el.style.width = el.dataset.pct + '%';
  });
  document.getElementById('analytics-flesch').innerHTML = `
    <div class="flesch-score" data-color="${flColor}">${flesch}</div>
    <div><div class="flesch-label" data-color="${flColor}">${flLabel}</div>
    <div class="u-fs-_7rem u-op-_7">/100 — Plus élevé = plus lisible</div></div>`;
  document.querySelectorAll('#analytics-flesch [data-color]').forEach(el => {
